...
func WrapV1(src io.ReadSeeker, dst io.Writer, opts ...Option) error {
	...
	v2Header := NewV2Header(uint64(v1Size))
	if _, err := dst.Write(V2Pragma); err != nil {
		return err
	}
...
func ExtractV1File(srcPath, dstPath string, opts ...Option) (err error) {
	...
	// Detect CAR version.
	version, err := ReadVersion(src, options.MaxAllowedHeaderSize)
	if err != nil {
		return err
	}
	if version == 1 {
	...
	if dataOffset < V2PragmaSize+V2HeaderSize {
...
func ReplaceRootsInFile(path string, roots []cid.Cid, opts ...Option) (err error) {
	...
	header := V1Header{}
	if _, err := header.ReadFromChecked(f, options.MaxAllowedHeaderSize); err != nil {
		return err
	}
	...
	case 2:
		...
		innerV1Header := V1Header{}
		if _, err = innerV1Header.ReadFromChecked(f, options.MaxAllowedHeaderSize); err != nil {
			return err
		}
	...
	if _, err = newHeader.WriteTo(&buf); err != nil {
		return err
	}
