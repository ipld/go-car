...
func TestPrepareTraversal(t *testing.T) {
	...
	h1, _ := car.OpenV2Reader("testdata/sample-unixfs-v2.car")
	h1h := bytes.Buffer{}
	h1h.Write(car.V2Pragma)
...
func TestPartialTraversal(t *testing.T) {
	...
	for err == nil {
		bm, err := loaded.SkipNext()
		if err == io.EOF {
			break
		}
		if _, ok := fnd[bm.Cid]; ok {
			require.Fail(t, "duplicate block present", bm.Cid)
		}
		fnd[bm.Cid] = struct{}{}
	}
