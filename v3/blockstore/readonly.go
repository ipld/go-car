...
type ReadOnly struct {
	...
	// If we called car.NewReaderMmap, remember to close it too.
...
var UseWholeCIDs = car.UseWholeCIDs
...
func NewReadOnly(backing io.ReaderAt, idx index.Index, opts ...car.Option) (*ReadOnly, error) {
	b := &ReadOnly{
		opts: car.ApplyOptions(opts...),
	}

	version, err := readVersion(backing, b.opts.MaxAllowedHeaderSize)
	if err != nil {
		return nil, err
	}
	switch version {
	case 1:
		...
		return b, nil
	case 2:
		v2r, err := car.NewV2Reader(backing, opts...)
		if err != nil {
			return nil, err
		}
		if idx == nil {
...
func readVersion(at io.ReaderAt, maxReadBytes uint64) (uint64, error) {
	var rr io.Reader
	switch r := at.(type) {
	case io.Reader:
		rr = r
	...
	return car.ReadVersion(rr, maxReadBytes)
}

func generateIndex(at io.ReaderAt, opts ...car.Option) (index.Index, error) {
	...
	return car.GenerateIndex(rs, opts...)
}
...
func (b *ReadOnly) AllKeysChan(ctx context.Context) (<-chan cid.Cid, error) {
	...
	header := car.V1Header{}
	headerSize, err := header.ReadFromChecked(rdr, b.opts.MaxAllowedHeaderSize)
	if err != nil {
		b.mu.RUnlock() // don't hold the mutex forever
	...
	if _, err = rdr.Seek(headerSize, io.SeekStart); err != nil {
		b.mu.RUnlock() // don't hold the mutex forever
		return nil, err
	}
...
func (b *ReadOnly) Roots() ([]cid.Cid, error) {
	...
	header := car.V1Header{}
	if _, err := header.ReadFromChecked(ors, b.opts.MaxAllowedHeaderSize); err != nil {
		return nil, fmt.Errorf("error reading car header: %w", err)
	}
