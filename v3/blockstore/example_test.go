...
func ExampleOpenReadWrite() {
	...
	rwbs, err := blockstore.OpenReadWrite(dst, roots, car.UseDataPadding(1413), car.UseIndexPadding(42))
	if err != nil {
		panic(err)
	}
	...
	// Finalize the blockstore to flush out the index and make a complete CAr.
	...
	resumedRwbos, err := blockstore.OpenReadWrite(dst, roots, car.UseDataPadding(1413))
	if err != nil {
		panic(err)
	}
	...
	// Finalize the blockstore to flush out the index and make a complete CAr.
	// Note, Finalize must be called on an open ReadWrite blockstore to flush out a complete CAr.
