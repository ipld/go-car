...
var WriteAsCarV1 = car.WriteAsCarV1
var AllowDuplicatePuts = car.AllowDuplicatePuts
...
func OpenReadWriteFile(f *os.File, roots []cid.Cid, opts ...car.Option) (*ReadWrite, error) {
	stat, err := f.Stat()
	if err != nil {
	...
	if resume {
		if err = store.ResumableVersion(f, rwbs.opts.MaxAllowedHeaderSize, rwbs.opts.WriteAsCarV1); err != nil {
			return nil, err
		}
		if err = store.Resume(
			f,
...
func (b *ReadWrite) initWithRoots(v2 bool, roots []cid.Cid) error {
	if v2 {
		if _, err := b.f.WriteAt(car.V2Pragma, 0); err != nil {
			return err
		}
	}
	_, err := car.V1Header{Roots: roots, Version: 1}.WriteTo(b.dataWriter)
	return err
}
...
func (b *ReadWrite) PutMany(ctx context.Context, blks []blocks.Block) error {
	...
	for _, bl := range blks {
		...
		if err := car.LengthPrefixedWrite(b.dataWriter, c.Bytes(), bl.RawData()); err != nil {
			return err
		}
