...
func TestBlockstore(t *testing.T) {
	...
	variants := []struct {
		...
		{"noopt_carv2", []car.Option{}, int64(car.V2PragmaSize + car.V2HeaderSize)},
		// option to only write as a CARv1, expect the noidentity inner CARv1
		{"carv1", []car.Option{blockstore.WriteAsCarV1(true)}, int64(0)},
	}
	...
	for _, variant := range variants {
		t.Run(variant.name, func(t *testing.T) {
			...
			r, err := testutil.NewV1Reader(f, false, 0, 0)
			require.NoError(t, err)
...
func TestBlockstoreResumption(t *testing.T) {
	...
	subject, err := blockstore.OpenReadWrite(path, r.Roots(),
		blockstore.UseWholeCIDs(true))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	wantPayloadReader, err := car.NewBlockReader(v1f)
	require.NoError(t, err)

	dr, err := v2r.DataReader()
	require.NoError(t, err)
	gotPayloadReader, err := car.NewBlockReader(dr)
	require.NoError(t, err)

	require.Equal(t, wantPayloadReader.Roots(), gotPayloadReader.Roots())
	require.Equal(t, wantPayloadReader.Version(), gotPayloadReader.Version())
	for {
...
func TestReadWriteWithPaddingWorksAsExpected(t *testing.T) {
	...
	gotCarV1, err := car.NewBlockReader(io.NewSectionReader(f, int64(wantCarV1Offset), int64(gotCarV2.Header.DataSize)))
	require.NoError(t, err)
	require.Equal(t, WantRoots, gotCarV1.Roots())
	gotOneCid, gotOneBytes, err := gotCarV1.Next()
	require.NoError(t, err)
	require.Equal(t, oneTestBlockWithCidV1.Cid(), gotOneCid)
	require.Equal(t, oneTestBlockWithCidV1.RawData(), gotOneBytes)
	gotAnotherCid, gotAnotherBytes, err := gotCarV1.Next()
	require.NoError(t, err)
	require.Equal(t, anotherTestBlockWithCidV0.Cid(), gotAnotherCid)
	require.Equal(t, anotherTestBlockWithCidV0.RawData(), gotAnotherBytes)
...
func TestOpenReadWrite_WritesIdentityCIDsWhenOptionIsEnabled(t *testing.T) {
	...
	subject, err := blockstore.OpenReadWrite(path, []cid.Cid{}, car.StoreIdentityCIDs(true))
	require.NoError(t, err)
	...
	reader, err := car.NewBlockReader(f)
	require.NoError(t, err)

	gotCid, gotBytes, err := reader.Next()
	require.NoError(t, err)
	require.Equal(t, idBlock.Cid(), gotCid)
	require.Equal(t, idBlock.RawData(), gotBytes)

	nextCid, nextBytes, err := reader.Next()
	require.Equal(t, io.EOF, err)
	require.Equal(t, cid.Undef, nextCid)
	require.Nil(t, nextBytes)

	// Assert the id is indexed.
	r, err := car.OpenV2Reader(path)
	require.NoError(t, err)
	...
	// Determine expected offset by reading the header
	dr, err := r.DataReader()
	require.NoError(t, err)
	header := car.V1Header{}
	expectedOffset, err := header.ReadFromChecked(dr, car.DefaultMaxAllowedHeaderSize)
	require.NoError(t, err)
...
func TestOpenReadWrite_ErrorsWhenWritingTooLargeOfACid(t *testing.T) {
	...
	subject, err := blockstore.OpenReadWrite(path, []cid.Cid{}, car.MaxIndexCidSize(maxAllowedCidSize))
	...
	require.Equal(t, &car.ErrCidTooLarge{MaxSize: maxAllowedCidSize, CurrentSize: bigCidLen}, err)
}
...
func TestReadWrite_ReWritingCARv1WithIdentityCidIsIdenticalToOriginalWithOptionsEnabled(t *testing.T) {
	...
	r, err := car.NewBlockReader(originalCarV1)
	require.NoError(t, err)

	path := filepath.Join(t.TempDir(), "readwrite-from-carv1-with-id-enabled.car")
	subject, err := blockstore.OpenReadWrite(path, r.Roots(), car.StoreIdentityCIDs(true))
	require.NoError(t, err)
	var idCidCount int
	for {
		nextCid, nextBytes, err := r.Next()
		if err == io.EOF {
			break
		}
		require.NoError(t, err)
		if nextCid.Prefix().MhType == multihash.IDENTITY {
			idCidCount++
		}
		nextBlock, err := blocks.NewBlockWithCid(nextBytes, nextCid)
		require.NoError(t, err)
		err = subject.Put(context.TODO(), nextBlock)
		require.NoError(t, err)
	}
...
func TestBlockstore_IdentityCidWithEmptyDataIsIndexed(t *testing.T) {
	...
	require.NoError(t, err)
	w, err := blockstore.OpenReadWrite(p, nil, car.StoreIdentityCIDs(true))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	header := car.V1Header{}
	wantOffset, err := header.ReadFromChecked(dr, car.DefaultMaxAllowedHeaderSize)
	require.NoError(t, err)
	...
	err = itidx.ForEach(func(m multihash.Multihash, u uint64) error {
		...
		require.Equal(t, uint64(wantOffset), u)
		count++
		return nil
	})
	require.NoError(t, err)
...
func TestWholeCID(t *testing.T) {
	for _, whole := range []bool{true, false} {
		...
		t.Run(fmt.Sprintf("whole=%t", whole), func(t *testing.T) {
			...
			rw, err := blockstore.OpenReadWrite(path, []cid.Cid{}, car.UseWholeCIDs(whole))
			require.NoError(t, err)
