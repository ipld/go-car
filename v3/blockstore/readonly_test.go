...
func TestReadOnly(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			[]car.Option{UseWholeCIDs(true), car.StoreIdentityCIDs(true)},
		...
		},
		{
			...
			[]car.Option{UseWholeCIDs(true), car.ZeroLengthSectionAsEOF(true)},
			false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			reader, err := car.NewBlockReader(f, tt.opts...)
			require.NoError(t, err)
			...
			for {
				wantCid, wantBytes, err := reader.Next()
				if err == io.EOF {
					break
				}
				require.NoError(t, err)

				wantCids = append(wantCids, wantCid)

				// Assert blockstore contains key.
				has, err := subject.Has(ctx, wantCid)
				require.NoError(t, err)
				...
				gotSize, err := subject.GetSize(ctx, wantCid)
				wantSize := len(wantBytes)
				require.NoError(t, err)
				require.Equal(t, wantSize, gotSize)

				// Assert block itself matches v1 payload block.
				if has {
					gotBlock, err := subject.Get(ctx, wantCid)
					require.NoError(t, err)
					require.Equal(t, wantCid, gotBlock.Cid())
					require.Equal(t, wantBytes, gotBlock.RawData())
				}

				blk, err := blocks.NewBlockWithCid(wantBytes, wantCid)
				require.NoError(t, err)
				// Assert write operations error
				require.Error(t, subject.Put(ctx, blk))
				require.Error(t, subject.PutMany(ctx, []blocks.Block{blk}))
				require.Error(t, subject.DeleteBlock(ctx, wantCid))
			}
...
func TestNewReadOnly_CarV1WithoutIndexWorksAsExpected(t *testing.T) {
	...
	v1r, err := car.NewBlockReader(reader)
	require.NoError(t, err)

	// Pick the first block in CARv1 as candidate to check `Get` works.
	wantCid, wantBytes, err := v1r.Next()
	require.NoError(t, err)
	...
	gotBlock, err := subject.Get(context.TODO(), wantCid)
	require.NoError(t, err)
	require.Equal(t, wantCid, gotBlock.Cid())
	require.Equal(t, wantBytes, gotBlock.RawData())
}
