...
func (w *writerOutput) Size() int64 {
	return w.size
}
...
func (w *writingReader) Read(p []byte) (int, error) {
	if w.wo != nil {
		...
		w.wo.rcrds[c] = index.Record{
			Cid:    c,
			Offset: uint64(w.wo.size),
		}
		w.wo.size += int64(w.len) + int64(len(size)+len(w.cid))
...
func TeeingLinkSystem(ls ipld.LinkSystem, w io.Writer, initialOffset int64, indexCodec multicodec.Code) (ipld.LinkSystem, IndexTracker) {
	wo := writerOutput{
		w:     w,
		size:  initialOffset,
		code:  indexCodec,
