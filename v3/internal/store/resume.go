...
func ResumableVersion(reader io.Reader, maxReadBytes uint64, writeAsV1 bool) error {
	version, err := car.ReadVersion(reader, maxReadBytes)
	if err != nil {
...
func Resume(
	...
	dataOffset uint64,
	v1 bool,
	maxAllowedV2HeaderSize uint64,
	zeroLengthSectionAsEOF bool,
) error {

	var headerInFile car.V2Header
	var v1r internalio.ReadSeekerAt

	if !v1 {
		if _, ok := rw.(interface{ Truncate(size int64) error }); !ok {
			return errors.New("cannot resume a CARv2 without the ability to truncate (e.g. an io.File)")
		}
		...
		r, err := internalio.NewOffsetReadSeeker(rw, car.V2PragmaSize)
		if err != nil {
			return err
		}
		...
		if err == nil && headerInFile.DataOffset != 0 {
			if headerInFile.DataOffset != dataOffset {
				...
				wantPadding := headerInFile.DataOffset - car.V2PragmaSize - car.V2HeaderSize
				gotPadding := dataOffset - car.V2PragmaSize - car.V2HeaderSize
				return fmt.Errorf(
	...
	header := car.V1Header{}
	if _, err := header.ReadFromChecked(v1r, maxAllowedV2HeaderSize); err != nil {
		// Cannot read the CARv1 header; the file is most likely corrupt.
		return fmt.Errorf("error reading car header: %w", err)
	}
	if !header.Matches(car.V1Header{Roots: roots, Version: 1}) {
	...
	if !v1 {
		...
		if _, err := new(car.V2Header).WriteTo(internalio.NewOffsetWriter(wat, car.V2PragmaSize)); err != nil {
	...
	for {
		...
		if length == 0 {
			if zeroLengthSectionAsEOF {
				...
				return errors.New("carv1 null padding not allowed by default; see WithZeroLegthSectionAsEOF")
			}
		}
		...
		if n > int(length) {
			return fmt.Errorf("section CID length %d exceeds section length %d", n, length)
		}
