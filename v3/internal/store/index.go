...
func FindCid(
	...
	err := idx.GetAll(key, func(offset uint64) bool {
		...
		var readCid cid.Cid
		if readBytes {
			readCid, fnData, err = car.ReadSection(reader, zeroLenAsEOF, maxReadBytes)
			if err != nil {
				fnErr = err
				return false
			}
...
func Finalize(writer io.WriterAt, header car.V2Header, idx *index.InsertionIndex, dataSize uint64, storeIdentityCIDs bool, indexCodec multicodec.Code) error {
	...
	if _, err := header.WriteTo(internalio.NewOffsetWriter(writer, car.V2PragmaSize)); err != nil {
		return err
	}
	return nil
}
