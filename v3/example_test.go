...
func ExampleWrapV1File() {
	...
	if err := car.WrapV1File(src, dst); err != nil {
		panic(err)
	}

	// Open our new CARv2 file and show some info about it.
	cr, err := car.OpenV2Reader(dst)
	if err != nil {
		panic(err)
	}
	defer func() {
...
func ExampleNewBlockReader() {
	for _, path := range []string{
		...
		fmt.Println("First 5 blocks:")
		for i := 0; i < 5; i++ {
			nextCid, nextBytes, err := br.Next()
			if err == io.EOF {
				break
			}
			if err != nil {
	...
	// Roots: [bafy2bzaced4ueelaegfs5fqu4tzsh6ywbbpfk3cxppupmxfdhbpbhzawfw5oy]
	// First 5 blocks:
	// 	bafy2bzaced4ueelaegfs5fqu4tzsh6ywbbpfk3cxppupmxfdhbpbhzawfw5oy (len=821)
	// 	bafy2bzaceaycv7jhaegckatnncu5yugzkrnzeqsppzegufr35lroxxnsnpspu (len=1053)
	// 	bafy2bzaceb62wdepofqu34afqhbcn4a7jziwblt2ih5hhqqm6zitd3qpzhdp4 (len=1094)
	// 	bafy2bzaceb3utcspm5jqcdqpih3ztbaztv7yunzkiyfq7up7xmokpxemwgu5u (len=1051)
	// 	bafy2bzacedjwekyjresrwjqj4n2r5bnuuu3klncgjo2r3slsp6wgqb37sz4ck (len=821)
