...
import (
	...
	"github.com/ipld/go-car/v3/internal/store"
	"github.com/ipld/go-car/v3/internal/testutil"
	"github.com/ipld/go-car/v3/storage"
...
func TestReadable(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			[]car.Option{car.UseWholeCIDs(true), car.StoreIdentityCIDs(true)},
			false,
		},
		{
			"OpenedWithCarV1_NoIdentityCID",
			"../testdata/sample-v1.car",
			[]car.Option{car.UseWholeCIDs(true)},
			false,
		},
		{
			"OpenedWithCarV2",
		...
		},
		{
			...
			[]car.Option{car.UseWholeCIDs(true), car.ZeroLengthSectionAsEOF(true)},
			false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			actual, err := car.NewBlockReader(actualReader, tt.opts...)
			require.NoError(t, err)
			...
			for {
				wantCid, wantBytes, err := actual.Next()
				if err == io.EOF {
					break
				}
				require.NoError(t, err)
				...
				has, err := readable.Has(ctx, wantCid.KeyString())
				require.NoError(t, err)
				if wantCid.Prefix().MhType == uint64(multicodec.Identity) && tt.noIdCids {
				...
				if has {
					gotBlock, err := readable.Get(ctx, wantCid.KeyString())
					require.NoError(t, err)
					require.Equal(t, wantBytes, gotBlock)

					reader, err := readable.GetStream(ctx, wantCid.KeyString())
					require.NoError(t, err)
			...
			storeIdentity := car.ApplyOptions(tt.opts...).StoreIdentityCIDs
			c = randIdentityCid()
...
func TestWritable(t *testing.T) {
	...
	variants := []struct {
		...
		{"carv2_noopt", "sample-v1-noidentity.car", []car.Option{}, int64(car.V2PragmaSize + car.V2HeaderSize)},
		{"carv2_identity", "sample-v1.car", []car.Option{car.StoreIdentityCIDs(true)}, int64(car.V2PragmaSize + car.V2HeaderSize)},
		{"carv1", "sample-v1-noidentity.car", []car.Option{car.WriteAsCarV1(true)}, int64(0)},
		{"carv1_identity", "sample-v1.car", []car.Option{car.WriteAsCarV1(true), car.StoreIdentityCIDs(true)}, int64(0)},
	}
		...
		t.Run(mode, func(t *testing.T) {
			for _, variant := range variants {
				t.Run(variant.name, func(t *testing.T) {
					...
					opts := car.ApplyOptions(variant.options...)
					...
					r, err := testutil.NewV1Reader(srcFile, false, 0, 0)
					require.NoError(t, err)
					...
					if mode == "WithoutRead" {
						...
						w, err := storage.NewWritable(writer, r.Roots(), variant.options...)
						require.NoError(t, err)
						...
						require.NoError(t, err)
						t.Cleanup(func() { dstFile.Close() })
						writable, err = storage.NewReadableWritable(dstFile, r.Roots(), variant.options...)
						require.NoError(t, err)
					}

					require.Equal(t, r.Roots(), writable.Roots())

					cids := make([]cid.Cid, 0)
					var idCidCount int
					for {
						// read from source
						wantCid, wantBytes, err := r.Next()
						if err == io.EOF {
							break
						}
						require.NoError(t, err)
						...
						// write to dest
						err = writable.Put(ctx, wantCid.KeyString(), wantBytes)
						require.NoError(t, err)
						cids = append(cids, wantCid)

						dmh, err := multihash.Decode(wantCid.Hash())
						require.NoError(t, err)
						...
						if mode == "WithRead" {
							...
							gotBytes, err := writable.Get(ctx, wantCid.KeyString())
							require.NoError(t, err)
							require.Equal(t, wantBytes, gotBytes)

							reader, err := writable.GetStream(ctx, wantCid.KeyString())
							require.NoError(t, err)
							data, err := io.ReadAll(reader)
							require.NoError(t, err)
							require.Equal(t, wantBytes, data)
					...
					rd, err := car.NewBlockReader(reopen)
					require.NoError(t, err)
...
func TestErrorsWhenWritingCidTooLarge(t *testing.T) {
	...
	subject, err := storage.NewWritable(out, []cid.Cid{}, car.MaxIndexCidSize(maxAllowedCidSize))
	require.NoError(t, err)
	...
	testCid, testData = randBlock()
	err = subject.Put(context.TODO(), testCid.KeyString(), testData)
	require.Equal(t, &car.ErrCidTooLarge{MaxSize: maxAllowedCidSize, CurrentSize: uint64(testCid.ByteLen())}, err)
}
...
func TestNullPadding(t *testing.T) {
	...
	readable, err := storage.OpenReadable(bufferReaderAt(paddedV1), car.ZeroLengthSectionAsEOF(true))
	require.NoError(t, err)
	...
	actual, err := car.NewBlockReader(bytes.NewReader(paddedV1), car.ZeroLengthSectionAsEOF(true))
	require.NoError(t, err)
	...
	for {
		...
		require.NoError(t, err)
		b, err := readable.Get(ctx, wantCid.KeyString())
		require.NoError(t, err)
...
func TestPutSameHashes(t *testing.T) {
	...
	wbsAllowDups, err := storage.NewReadableWritable(dstAllowDups, nil, car.AllowDuplicatePuts(true))
	require.NoError(t, err)
	...
	wbsByCID, err := storage.NewReadableWritable(dstByCID, nil, car.UseWholeCIDs(true))
	require.NoError(t, err)
...
func TestWritableCantRead(t *testing.T) {
	...
	writable, err := storage.NewWritable(&writerOnly{out}, nil, car.WriteAsCarV1(true))
	require.NoError(t, err)
...
func TestReadWriteWithPaddingWorksAsExpected(t *testing.T) {
	...
	subject, err := storage.NewReadableWritable(
		writer,
		wantRoots,
		car.UseDataPadding(wantCarV1Padding),
		car.UseIndexPadding(wantIndexPadding))
	require.NoError(t, err)
	...
	gotCarV2, err := car.OpenV2Reader(path)
	t.Cleanup(func() { gotCarV2.Close() })
	require.NoError(t, err)
	wantCarV1Offset := car.V2PragmaSize + car.V2HeaderSize + wantCarV1Padding
	...
	gotCarV1, err := testutil.NewV1Reader(io.NewSectionReader(f, int64(wantCarV1Offset), int64(gotCarV2.Header.DataSize)), false, 0, 0)
	require.NoError(t, err)
	require.Equal(t, wantRoots, gotCarV1.Roots())
	gotCid, gotBytes, err := gotCarV1.Next()
	require.NoError(t, err)
	require.Equal(t, testCid1, gotCid)
	require.Equal(t, testData1, gotBytes)
	gotCid, gotBytes, err = gotCarV1.Next()
	require.NoError(t, err)
	require.Equal(t, testCid2, gotCid)
	require.Equal(t, testData2, gotBytes)

	_, _, err = gotCarV1.Next()
...
func TestResumption(t *testing.T) {
	...
	subject, err := storage.NewReadableWritable(writer, roots, car.UseWholeCIDs(true))
	require.NoError(t, err)
	...
	for b := range blockSource() {
		...
		if resume {
			...
			require.NoError(t, err)
			subject, err = storage.OpenReadableWritable(writer, roots, car.UseWholeCIDs(true))
			require.NoError(t, err)
		}
	...
	require.NoError(t, err)
	wantPayloadReader, err := testutil.NewV1Reader(v1f, false, 0, 0)
	require.NoError(t, err)

	dr, err := v2r.DataReader()
	require.NoError(t, err)
	gotPayloadReader, err := testutil.NewV1Reader(dr, false, 0, 0)
	require.NoError(t, err)

	require.Equal(t, wantPayloadReader.Roots(), gotPayloadReader.Roots())
	for {
		wantNextCid, wantNextBytes, wantErr := wantPayloadReader.Next()
		if wantErr == io.EOF {
			gotNextCid, gotNextBytes, gotErr := gotPayloadReader.Next()
			require.Equal(t, wantErr, gotErr)
			require.Equal(t, cid.Undef, gotNextCid)
			require.Nil(t, gotNextBytes)
			break
		}
		...
		require.Equal(t, wantNextCid, gotNextCid)
		require.Equal(t, wantNextBytes, gotNextBytes)
	}
	...
	dr, err = v2r.DataReader()
	require.NoError(t, err)
	wantIdx, err := car.GenerateIndex(dr)
	require.NoError(t, err)
...
func TestResumptionV1(t *testing.T) {
	...
	blockSource := func() <-chan simpleBlock {
		...
		require.NoError(t, err)
		t.Cleanup(func() { require.NoError(t, v1f.Close()) })
		r, err := testutil.NewV1Reader(v1f, false, 0, 0)
		require.NoError(t, err)
		ret := make(chan simpleBlock)
	...
	t.Cleanup(func() { require.NoError(t, writer.Close()) })
	// Create an incomplete CARv2 file with no blocks put.
	subject, err := storage.NewReadableWritable(writer, roots, car.UseWholeCIDs(true), car.WriteAsCarV1(true))
	require.NoError(t, err)
	...
	for b := range blockSource() {
		...
		if resume {
			...
			_, err := writer.Seek(0, io.SeekStart)
			require.NoError(t, err)
			subject, err = storage.OpenReadableWritable(writer, roots, car.UseWholeCIDs(true), car.WriteAsCarV1(true))
			require.NoError(t, err)
		}
		require.NoError(t, subject.Put(ctx, b.cid.KeyString(), b.data))
...
func TestWholeCID(t *testing.T) {
	for _, whole := range []bool{true, false} {
		...
		t.Run(fmt.Sprintf("whole=%t", whole), func(t *testing.T) {
			...
			require.NoError(t, err)
			t.Cleanup(func() { require.NoError(t, out.Close()) })
			store, err := storage.NewReadableWritable(out, []cid.Cid{}, car.UseWholeCIDs(whole))
			require.NoError(t, err)
...
func TestIndex(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			"IndexCarV1",
			"../testdata/sample-v1.car",
			testutil.ListCids(t, testutil.NewV1ReaderFromV1File(t, "../testdata/sample-v1.car", false)),
		},
		{
			"IndexCarV2",
			"../testdata/sample-wrapped-v2.car",
			testutil.ListCids(t, testutil.NewV1ReaderFromV2File(t, "../testdata/sample-wrapped-v2.car", false)),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			t.Cleanup(func() { require.NoError(t, f.Close()) })
			subject, err := storage.OpenReadable(f, car.UseWholeCIDs(true))
			require.NoError(t, err)
