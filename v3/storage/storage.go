...
func OpenReadable(reader io.ReaderAt, opts ...car.Option) (ReadableCar, error) {
	sc := &StorageCar{opts: car.ApplyOptions(opts...)}

	rr := internalio.ToReadSeeker(reader)
	header := car.V1Header{}
	if _, err := header.ReadFromChecked(rr, sc.opts.MaxAllowedHeaderSize); err != nil {
		return nil, err
	}
	...
	case 1:
		...
		if err := car.LoadIndex(sc.idx, rr, opts...); err != nil {
			return nil, err
		}
	case 2:
		v2r, err := car.NewV2Reader(reader, opts...)
		if err != nil {
			return nil, err
		}
		...
		if v2r.Header.HasIndex() {
			...
			sc.idx = index.NewInsertionIndex()
			if err := car.LoadIndex(sc.idx, dr, opts...); err != nil {
				return nil, err
			}
		}
...
func NewWritable(writer io.Writer, roots []cid.Cid, opts ...car.Option) (WritableCar, error) {
...
func newWritable(writer io.Writer, roots []cid.Cid, opts ...car.Option) (*StorageCar, error) {
	sc := &StorageCar{
		writer: &positionTrackingWriter{w: writer},
		idx:    index.NewInsertionIndex(),
		header: car.NewV2Header(0),
		opts:   car.ApplyOptions(opts...),
		roots:  roots,
	}
	...
	if writerAt, ok := writer.(io.WriterAt); ok {
		...
		if !sc.opts.WriteAsCarV1 {
			return nil, errors.New("cannot write as CARv2 to a non-seekable writer")
		}
	}
...
func newReadableWritable(rw ReaderAtWriterAt, roots []cid.Cid, opts ...car.Option) (*StorageCar, error) {
...
func OpenReadableWritable(rw ReaderAtWriterAt, roots []cid.Cid, opts ...car.Option) (*StorageCar, error) {
	sc, err := newReadableWritable(rw, roots, opts...)
	if err != nil {
		return nil, err
	}
	...
	if err := store.ResumableVersion(rs, sc.opts.MaxAllowedHeaderSize, sc.opts.WriteAsCarV1); err != nil {
		return nil, err
	}
	if err := store.Resume(
		rw,
...
func (sc *StorageCar) init() (WritableCar, error) {
	if !sc.opts.WriteAsCarV1 {
		if _, err := sc.writer.Write(car.V2Pragma); err != nil {
			return nil, err
		}
	}
	...
	if _, err := (car.V1Header{Roots: sc.roots, Version: 1}.WriteTo(w)); err != nil {
		return nil, err
	}
	return sc, nil
}
...
func (sc *StorageCar) Put(ctx context.Context, keyStr string, data []byte) error {
	...
	if !ok || sc.writer == nil {
		return errors.New("cannot put into a read-only CAR")
	}
	...
	n := uint64(w.Position())
	if err := car.LengthPrefixedWrite(w, keyCid.Bytes(), data); err != nil {
		return err
	}
...
func (sc *StorageCar) GetStream(ctx context.Context, keyStr string) (io.ReadCloser, error) {
	if sc.reader == nil {
		return nil, errors.New("cannot read from a write-only CAR")
	}
...
func (sc *StorageCar) Finalize() error {
	...
	if !ok { // should should already be checked at construction if this is a writable
		return errors.New("cannot finalize a CARv2 without an io.WriterAt")
	}
	...
	if sc.closed {
		...
		return errors.New("called Finalize on a closed storage CAR")
	}
