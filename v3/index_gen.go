...
// See: LoadIndex.
func GenerateIndex(r io.Reader, opts ...Option) (index.Index, error) {
	wopts := ApplyOptions(opts...)
	idx, err := index.New(wopts.IndexCodec)
	if err != nil {
		return nil, err
	}
	if err := LoadIndex(idx, r, opts...); err != nil {
		return nil, err
	}
	return idx, nil
}
...
func LoadIndex(idx index.Index, r io.Reader, opts ...Option) error {
	...
	pragma := V1Header{}
	if _, err := pragma.ReadFromChecked(r, o.MaxAllowedHeaderSize); err != nil {
	...
	case 2:
		...
		if v2h.DataOffset < V2HeaderSize {
		...
		v1h := V1Header{}
		if _, err := v1h.ReadFromChecked(reader, o.MaxAllowedHeaderSize); err != nil {
			return err
		}
		if v1h.Version != 1 {
	...
	for {
		...
		if sectionLen == 0 {
			if o.ZeroLengthSectionAsEOF {
				...
				return errors.New("carv1 null padding not allowed by default; see ZeroLengthSectionAsEOF")
			}
		}
...
func ReadOrGenerateIndex(rs io.ReadSeeker, opts ...Option) (index.Index, error) {
	// Read version.
	ropts := ApplyOptions(opts...)
	version, err := ReadVersion(rs, ropts.MaxAllowedHeaderSize)
	if err != nil {
		return nil, err
	}
	...
	case 2:
		// Read CARv2 format
		v2r, err := NewV2Reader(internalio.ToReaderAt(rs), opts...)
		if err != nil {
			return nil, err
		}
