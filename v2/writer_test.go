...
func TestWrapV1Withmessage(t *testing.T) {
	dagSvc := dstest.Mock()
	blk := merkledag.NewRawNode([]byte("random meaningless bytes"))
	require.NoError(t, dagSvc.Add(context.Background(), blk))
	src := filepath.Join(t.TempDir(), "messaging-actual-unwrapped.car")
	sf, err := os.Create(src)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, sf.Close()) })
	require.NoError(t, carv1.WriteCar(context.Background(), dagSvc, []cid.Cid{blk.Cid()}, sf))

	dest := filepath.Join(t.TempDir(), "messaging-actual-wrapped.car")
	df, err := os.Create(dest)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, df.Close()) })
	_, err = sf.Seek(0, io.SeekStart)
	...
	node, err := qp.BuildMap(basicnode.Prototype.Any, -1, func(ma datamodel.MapAssembler) {
		qp.MapEntry(ma, "expectedRoot", qp.Link(cidlink.Link{Cid: blk.Cid()}))
		qp.MapEntry(ma, "sneaky", qp.String("sending a message outside of CARv1 payload"))
	})
	require.NoError(t, err)
	require.NoError(t, WrapV1(sf, df, WithoutIndex(), EmbedMessage(node)))

	_, err = df.Seek(0, io.SeekStart)
	require.NoError(t, err)
	gotCar, err := ioutil.ReadAll(df)
	require.NoError(t, err)
	wf, err := os.Open("testdata/messaging.car") // generated by js-car
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, wf.Close()) })
	require.NoError(t, err)
	wantCar, err := ioutil.ReadAll(wf)
	require.NoError(t, err)
	require.Equal(t, wantCar, gotCar)
}
